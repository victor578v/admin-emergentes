...
function NovaPlataforma() {
  const { register, handleSubmit, reset } = useForm<Inputs>()

  // Função para incluir uma nova plataforma
  async function incluirPlataforma(data: Inputs) {
    ...
    if (response.status === 201) {
      ...
      toast.error("Erro ao cadastrar plataforma")
    }
  }
  ...
  return (
    <>
      ...
      <form className="max-w-xl mx-auto" onSubmit={handleSubmit(incluirPlataforma)}>
        <div className="mb-3">
          <label htmlFor="nome" className="block mb-2 text-sm font-medium text-gray-900">
            Nome da Plataforma</label>
          <input
            type="text"
            id="nome"
